...
export function PricingFeedbackBadge() {
  ...
  async function submit() {
    ...
    try {
      const response = await apiFetch("/api/client/feedback", {
        ...
        body: JSON.stringify({ savingsValue: savings, monthlyPrice: price }),
      });
  ...
  const badge = (
    <button
      ...
      className="fixed right-0 top-[42%] z-[60] flex items-center gap-1.5 rounded-l-xl border border-r-0 border-primary/15 bg-primary px-2.5 py-2 text-xs font-semibold text-primary-foreground shadow-lg"
  ...
  const sheet = (
    <div className="fixed inset-0 z-[1000] flex items-end justify-center bg-black/35" onClick={() => setOpen(false)}>
      <section
        className="w-full max-w-[460px] rounded-t-[2rem] bg-background px-5 pb-[max(1.5rem,env(safe-area-inset-bottom))] pt-5 shadow-2xl"
        onClick={(event) => event.stopPropagation()}
        role="dialog"
        aria-modal="true"
        aria-label="Kurze LocalPrices-Abstimmung"
      >
        <div className="mb-4 flex items-start justify-between gap-3">
          <div>
            <p className="text-xs font-semibold uppercase tracking-[0.12em] text-primary">Kurze Abstimmung</p>
            <h2 className="mt-1 font-display text-2xl font-semibold text-foreground">Hilf uns, LocalPrices besser zu machen</h2>
          </div>
          <button type="button" onClick={() => setOpen(false)} className="rounded-full p-2 text-muted-foreground hover:bg-muted" aria-label="Schließen">
            <X className="h-5 w-5" />
          </button>
        </div>

        <p className="mb-5 text-sm leading-6 text-muted-foreground">
          LocalPrices soll dir helfen, durch günstigere Angebote, passende Märkte und einen besseren Einkaufsplan regelmäßig Geld zu sparen.
        </p>

        {step === 1 ? (
          <div>
            <h3 className="mb-3 text-base font-semibold">Hilft dir LocalPrices beim Sparen?</h3>
            <div className="space-y-2">
              {SAVINGS.map((item) => (
                <button
                  ...
                  type="button"
                  onClick={() => {
                    setSavings(item.value);
                    setStep(2);
                  }}
                  className="flex w-full items-center justify-between rounded-2xl border border-border bg-surface px-4 py-3 text-left text-sm font-medium hover:border-primary/40"
                >
                  {item.label}
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-2">
              {PRICES.map((item) => (
                <button
                  ...
                  className={`rounded-2xl border px-3 py-3 text-sm font-semibold ${price === item.value ? "border-primary bg-primary/10 text-primary" : "border-border bg-surface text-foreground"} ${item.value === "none" ? "col-span-2" : ""}`}
                >
                  {item.label}
                </button>
              ))}
            ...
            <button
              ...
              className="mt-4 flex w-full items-center justify-center gap-2 rounded-2xl bg-primary px-4 py-3.5 text-sm font-semibold text-primary-foreground disabled:opacity-50"
            >
              <CheckCircle2 className="h-4 w-4" /> {saving ? "Speichere …" : "Antwort senden"}
            </button>
          </div>
        )}
      </section>
