...
export function OptimizedTripSummary({ trip }: { trip: OptimizedTrip }) {
  ...
  return (
    <section className="gradient-savings rounded-2xl p-4 text-white shadow-raised">
      <p className="flex items-center gap-1.5 text-[14px] font-semibold"><CheckCircle2 className="h-4 w-4" /> Dein optimaler Einkauf</p>
      <div className="mt-3 grid grid-cols-4 gap-1.5">
        {kpis.map((k) => <div key={k.label} className="rounded-xl bg-white/12 px-1 py-2 text-center"><p className={cn("tabular text-[13px] font-bold leading-tight", k.highlight && "text-[#BBF7D0]")}>{k.value}</p><p className="mt-0.5 text-[9px] leading-tight text-white/75">{k.label}</p></div>)}
      </div>
      <div className="mt-3 rounded-xl bg-white/12 px-3 py-2 text-center">
        {noSingleStoreComparison ? <><p className="text-[12px] font-semibold">{trip.stops.length} Märkte nötig für alle aktuellen Angebotsartikel</p><p className="mt-0.5 flex items-center justify-center gap-1 text-[10px] text-white/75"><Info className="h-3 w-3" /> Kein einzelner Markt deckt aktuell alle {coveredItemCount} preislich vergleichbaren Artikel ab.</p></> : <><p className="text-[12px] font-semibold">{trip.combinationLabel} spart dir {formatEuro(trip.savings)}</p><p className="mt-0.5 flex items-center justify-center gap-1 text-[10px] text-white/75"><Info className="h-3 w-3" /> {trip.savingsExplanation}</p></>}
      </div>
      {coveredItemCount < totalItemCount && <p className="mt-2 text-center text-[10px] text-white/75">{coveredItemCount} von {totalItemCount} Artikeln haben aktuell einen Marktpreis; die übrigen bleiben auf deiner Liste.</p>}
    </section>
  );
}
...
export function TripRouteRow({ trip }: { trip: OptimizedTrip }) {
  ...
  const markets = trip.stops.map((stop) => getMarket(stop.marketId));
  const stops = trip.stops.map((stop, index) => markets[index]?.name ?? stop.marketName ?? "Markt");
  const homeDistances = markets.map((market) => market?.distanceKm ?? 0);
  const sectionKm: number[] = [];
  if (stops.length === 1) {
    sectionKm.push(homeDistances[0] || trip.distanceKm / 2, Math.max(0, trip.distanceKm - (homeDistances[0] || trip.distanceKm / 2)));
  } else if (stops.length >= 2) {
    const first = homeDistances[0] || 0;
    const back = homeDistances[homeDistances.length - 1] || 0;
    const remaining = Math.max(0, trip.distanceKm - first - back);
    sectionKm.push(first);
    const internalCount = stops.length - 1;
    for (let i = 0; i < internalCount; i += 1) sectionKm.push(remaining / internalCount);
    sectionKm.push(back);
  }
  const route = ["Zuhause", ...stops, "Zuhause"];

  return (
    <div className="card-surface overflow-hidden text-[12px] text-muted-foreground">
      <button type="button" onClick={() => setOpen((value) => !value)} aria-expanded={open} className="tabular flex w-full items-center gap-2 px-3 py-2.5 text-left"><Car className="h-4 w-4 shrink-0 text-info" /><span className="min-w-0 flex-1">Fahrtkosten: {formatEuro(trip.travelCost)} · {trip.stops.length} Stopps · {trip.distanceKm.toLocaleString("de-DE", { maximumFractionDigits: 1 })} km</span><ChevronDown className={cn("h-4 w-4 shrink-0 transition-transform", open && "rotate-180")} /></button>
      {open && <div className="border-t border-border px-3 py-3">
        <p className="mb-2 text-[10px] font-semibold uppercase tracking-wide text-muted-foreground">Rundtour</p>
        <div className="space-y-2">{route.map((label, index) => <div key={`${label}-${index}`} className="flex items-center gap-2"><span className={cn("grid h-6 w-6 shrink-0 place-items-center rounded-full", index === 0 || index === route.length - 1 ? "bg-info/10 text-info" : "bg-primary-soft text-primary")}>{index === 0 || index === route.length - 1 ? <Car className="h-3.5 w-3.5" /> : <MapPin className="h-3.5 w-3.5" />}</span><span className="min-w-0 flex-1 truncate text-[11px] font-medium text-navy">{label}</span>{index > 0 && <span className="tabular text-[10px] font-medium text-muted-foreground">{(sectionKm[index - 1] ?? 0).toLocaleString("de-DE", { maximumFractionDigits: 1 })} km</span>}</div>)}</div>
        <p className="mt-2 text-[10px] leading-relaxed text-muted-foreground">Die Abschnitte basieren auf den Straßenentfernungen der Märkte und der berechneten Gesamtrundtour. Bei mehreren Zwischenmärkten wird der verbleibende Streckenanteil auf die Zwischenabschnitte verteilt.</p>
      </div>}
    </div>
  );
}
