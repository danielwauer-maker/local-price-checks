...
export function FavoriteCard({
  ...
  onToggleAlternatives?: (value: boolean) => void;
}) {
  return (
    <article className="card-surface p-3">
      <div className="flex items-center gap-3">
        ...
        >
          <p className="line-clamp-2 text-[13px] font-semibold text-navy">{product.name}</p>
        </Link>
        <button
          type="button"
          ...
          className="tap-target grid shrink-0 place-items-center rounded-xl text-discount"
        >
          <Heart
            className={cn(
              "h-5 w-5 transition-all duration-150 text-discount",
              isFavorite && "fill-discount",
            )}
          />
        </button>
      </div>
      {onToggleAlternatives && (
        <div className="mt-3 flex items-center justify-between gap-3 border-t border-border pt-3">
          ...
          <button
            type="button"
            role="switch"
            aria-label="Alternativen zulassen"
            aria-checked={allowAlternatives}
            onClick={() => onToggleAlternatives(!allowAlternatives)}
            className={cn(
              "relative h-6 w-11 shrink-0 overflow-hidden rounded-full border-0 p-0 transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/40",
              allowAlternatives ? "bg-primary" : "bg-muted-surface",
            )}
          >
            <span
              className={cn(
