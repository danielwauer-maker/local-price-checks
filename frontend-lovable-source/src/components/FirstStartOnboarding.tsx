  ...
  return (
    <div className="relative h-full w-full overflow-hidden rounded-[1.75rem] border border-primary/10 bg-primary/[0.045] p-4" aria-hidden="true">
      ...
      <div className="absolute bottom-3 right-3 rounded-xl bg-surface/95 px-3 py-2 text-left shadow-sm ring-1 ring-border">
        <p className="text-[9px] font-medium text-muted-foreground">In deiner Nähe</p>
        <p className="mt-0.5 text-[11px] font-semibold text-navy">3 Märkte gefunden</p>
      </div>
    </div>
  );
}
    ...
    <div className="h-full w-full rounded-[1.75rem] border border-primary/10 bg-primary/[0.045] p-4" aria-hidden="true">
      <div className="mx-auto h-full max-w-[300px] overflow-hidden rounded-2xl border border-border bg-surface shadow-sm">
        <div className="flex items-center justify-between bg-muted-surface px-3 py-2.5">
          ...
          <span className="text-[9px] text-muted-foreground">3 Artikel</span>
        </div>
        <div className="divide-y divide-border">
          {rows.map((row, index) => (
            <div key={row} className="flex items-center gap-2.5 px-3 py-2.5">
              <span className={`grid h-5 w-5 shrink-0 place-items-center rounded-full border-2 ${index === 0 ? "border-primary bg-primary text-primary-foreground" : "border-border"}`}>
              ...
              {index === 1 && <Heart className="h-4 w-4 fill-primary/15 text-primary" />}
            </div>
          ))}
        </div>
      </div>
    ...
    <div className="h-full w-full rounded-[1.75rem] border border-primary/10 bg-primary/[0.045] p-4" aria-hidden="true">
      <div className="mx-auto grid h-full max-w-[310px] grid-cols-[1fr_auto_1fr] items-center gap-2">
        <div className="rounded-2xl border border-border bg-surface p-3 text-center shadow-sm">
          ...
          <p className="mt-1 text-[15px] font-bold text-navy">28,40 €</p>
        </div>
        <TrendingDown className="h-5 w-5 text-primary" />
        <div className="rounded-2xl border border-primary/20 bg-surface p-3 text-center shadow-sm ring-2 ring-primary/5">
          <Store className="mx-auto h-5 w-5 text-primary" />
          <p className="mt-2 text-[10px] font-semibold text-navy">Bessere Wahl</p>
...
const CARDS: OnboardingCard[] = [
  ...
  {
    eyebrow: "Dein Einkauf",
    title: "Favoriten & Einkaufsliste",
    text: "Merke dir Produkte, füge Artikel zur Liste hinzu und hake sie beim Einkauf bequem ab. So bleibt alles übersichtlich.",
    visual: <ListVisual />,
  },
  {
...
function rememberCompletion(): void {
  try {
    ...
    // Storage may be unavailable in privacy-restricted contexts. Closing the
    // overlay must still work for the current session.
  }
}
...
export function FirstStartOnboarding({ active = true }: { active?: boolean }) {
  ...
  return createPortal(
    <div
      className="fixed inset-0 isolate bg-background"
      style={{ zIndex: 2147483647 }}
      role="dialog"
      aria-modal="true"
      aria-label="Einführung in Spareno"
    >
      <div className="mx-auto flex h-[100dvh] w-full max-w-[480px] flex-col overflow-hidden bg-background px-4 pb-[max(0.9rem,env(safe-area-inset-bottom))] pt-[max(0.75rem,env(safe-area-inset-top))] sm:px-5">
        ...
        <div
          className="flex min-h-0 flex-1 touch-pan-y select-none flex-col"
