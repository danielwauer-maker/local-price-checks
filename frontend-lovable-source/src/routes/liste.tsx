...
function ListScreen() {
  ...
  const {
    listEntries, manualListItems, checkedListItems, setQty, addToList, addManualListItem,
    setManualListQty, removeManualListItem, clearList, toggleListChecked, sharingEnabled,
    activeShoppingListIsPersonal, activeShoppingListMembers,
  } = useStore();

  useEffect(() => {
    const timer = window.setTimeout(() => setSuggestionQuery(manualName.trim()), 220);
    return () => window.clearTimeout(timer);
  }, [manualName]);

  const suggestionsQuery = useQuery({
    queryKey: ["shopping-offer-suggestions", suggestionQuery.toLocaleLowerCase("de-DE")],
    queryFn: () => fetchOffers({ query: suggestionQuery }),
    enabled: suggestionQuery.length >= 2,
    staleTime: 30_000,
    gcTime: 5 * 60_000,
  });
  const suggestions = useMemo(() => cheapestPerProduct(suggestionsQuery.data ?? []), [suggestionsQuery.data]);
  const showSuggestions = inputFocused && manualName.trim().length >= 2;

  const features = useQuery({ queryKey: ["lokero-features"], queryFn: fetchFeatures, staleTime: 5 * 60_000 });
  const trip = useQuery({ queryKey: ["optimized-trip"], queryFn: fetchOptimizedTrip, staleTime: 30_000 });
  ...
  const groups = useMemo<ListGroup[]>(() => {
    ...
    for (const entry of listEntries) {
      ...
      if (!best || !market) { unknown.entries.push(entry); continue; }
  ...
  const startVoiceInput = () => {
    ...
    if (isIOS) { toast.info("Spracheingabe ist auf iPhone und iPad aktuell deaktiviert. Du kannst Artikel weiterhin über die iOS-Diktierfunktion der Tastatur einsprechen."); return; }
    ...
    try {
      ...
      recognition.onerror = () => { setIsListening(false); recognitionRef.current = null; toast.error("Spracheingabe konnte nicht erkannt werden."); };
      recognition.onend = () => { setIsListening(false); recognitionRef.current = null; };
      recognitionRef.current = recognition; recognition.start(); setIsListening(true);
    } catch { recognitionRef.current = null; setIsListening(false); toast.error("Spracheingabe konnte nicht gestartet werden."); }
  };
    ...
    <div>
      ...
      {tab === "optimiert" && (
        <div className="space-y-3 px-4 pt-4">
          ...
          {collaborativeListActive && <section className="card-surface p-5 text-center"><span className="mx-auto grid h-10 w-10 place-items-center rounded-xl bg-primary-soft text-primary"><Users className="h-5 w-5" /></span><h2 className="mt-3 text-[15px] font-semibold text-navy">Optimierung für gemeinsame Listen folgt</h2><p className="mx-auto mt-1 max-w-[310px] text-[12px] leading-relaxed text-muted-foreground">Diese Liste wird live mit anderen Personen synchronisiert. Damit Spareno dir keine falsche Marktkombination zeigt, ist die Einkaufsoptimierung für gemeinsame Listen vorerst bewusst deaktiviert.</p><button type="button" onClick={() => changeTab("meine")} className="mt-4 h-11 rounded-xl bg-primary px-4 text-[13px] font-semibold text-primary-foreground">Gemeinsame Liste ansehen</button></section>}
          {!collaborativeListActive && !features.isLoading && !trip.isLoading && !optimizationEnabled && <section className="card-surface p-5 text-center"><span className="mx-auto grid h-10 w-10 place-items-center rounded-xl bg-primary-soft text-primary"><Clock3 className="h-5 w-5" /></span><h2 className="mt-3 text-[15px] font-semibold text-navy">Einkaufsoptimierung wird vorbereitet</h2><p className="mx-auto mt-1 max-w-[300px] text-[12px] leading-relaxed text-muted-foreground">Deine Einkaufsliste funktioniert bereits. Sobald mehrere geprüfte Märkte und belastbare Vergleichspreise verfügbar sind, schaltet Spareno die optimale Marktkombination frei.</p><button type="button" onClick={() => changeTab("meine")} className="mt-4 h-11 rounded-xl bg-primary px-4 text-[13px] font-semibold text-primary-foreground">Liste ansehen</button></section>}
      ...
      {tab === "meine" && (
        <div className="space-y-4 px-4 pt-4">
          <form onSubmit={submitManualItem} className="relative rounded-2xl border border-border bg-surface p-3">
            <label htmlFor="manual-list-item" className="mb-2 block text-[12px] font-semibold text-navy">Artikel hinzufügen</label>
            <div className="flex gap-2">
              <input id="manual-list-item" value={manualName} onChange={(event) => setManualName(event.target.value)} onFocus={() => setInputFocused(true)} onBlur={() => window.setTimeout(() => setInputFocused(false), 160)} placeholder="z. B. Butter, Milch, Müllbeutel …" maxLength={80} autoComplete="off" enterKeyHint="done" className="h-11 min-w-0 flex-1 rounded-xl border border-border bg-muted-surface px-3 text-[13px] text-navy outline-none placeholder:text-muted-foreground focus:border-primary" />
