...
type BackendPlan = {
  ...
  offeredItems: number;
  totalItems: number;
  missing: Array<{ product: (typeof PRODUCTS)[number]; qty: number }>;
};

const EMPTY_PLAN: BackendPlan = {
  stops: [],
  total: 0,
  merchandiseTotal: 0,
  travelKm: 0,
...
function ListPage() {
  const { basketEntries, setQty, addToBasket, clearBasket, maxStops, setMaxStops, hydrated } = useStore();
              ...
              <li key={p.id}>
                ...
                >
                  ...
                  <span className="min-w-0 flex-1">
                    ...
                    <span className="block text-xs text-muted-foreground">{p.brand} · {p.unit}</span>
                  </span>
        ...
        <>
          <section className="mx-5 mt-5 overflow-hidden rounded-3xl gradient-hero p-5 text-primary-foreground shadow-float">
            ...
            <p className="mt-1 text-sm opacity-85">
              {plan.offeredItems} von {plan.totalItems} Artikeln im Angebot · {plan.stops.length} {plan.stops.length === 1 ? "Markt" : "Märkte"}
            </p>
            <div className="mt-4 grid grid-cols-2 gap-2 text-sm">
              <div className="rounded-2xl bg-white/12 px-3 py-2">
                <span className="block text-[11px] opacity-80">Waren</span>
                <span className="tabular font-bold">{formatEuro(plan.merchandiseTotal)}</span>
              </div>
              <div className="rounded-2xl bg-white/12 px-3 py-2">
                <span className="block text-[11px] opacity-80">Fahrt · {plan.travelKm.toFixed(1)} km</span>
                <span className="tabular font-bold">{formatEuro(plan.travelCost)}</span>
              </div>
            </div>
            {plan.singleMarket && (
              <p className="mt-3 text-xs opacity-85">
                Gegenüber bestem Einzelmarkt {plan.savingsVsSingle > 0 ? `${formatEuro(plan.savingsVsSingle)} günstiger` : "kein Mehrmarkt-Vorteil"}.
              </p>
            )}
          </section>
              ...
              {[1, 2, 3].map((n) => (
                <button
                  ...
                  className={cn(
                    ...
                    maxStops === n ? "bg-primary text-primary-foreground" : "bg-secondary text-secondary-foreground",
                  )}
                >
                  {n}
                </button>
              ...
              <article key={stop.market.id} className="surface-card overflow-hidden">
                <header className="flex items-center gap-3 border-b border-border px-4 py-3">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="text-[11px] text-muted-foreground">{stop.market.city}</p>
                  </div>
                  <span className="tabular text-sm font-bold">{formatEuro(stop.total)}</span>
                </header>
                <ul>
                  {stop.lines.map((line) => (
                    <li key={line.product.id} className="flex items-center gap-3 border-b border-border px-4 py-2.5 last:border-0">
                      <span className="text-xl">{line.product.emoji}</span>
                      <div className="min-w-0 flex-1">
                        <p className="truncate text-sm font-medium">{line.product.name}</p>
                        <p className="text-[11px] text-muted-foreground">
                          {line.product.unit}
                          {line.isOffer && <span className="ml-1.5 rounded-full bg-deal/12 px-1.5 py-0.5 font-semibold text-deal">Angebot</span>}
                        </p>
                      </div>
                      <div className="flex items-center gap-1.5">
                        <button onClick={() => setQty(line.product.id, line.qty - 1)} className="flex h-7 w-7 items-center justify-center rounded-full bg-secondary text-secondary-foreground" aria-label="weniger"><Minus className="h-3.5 w-3.5" /></button>
                        <span className="tabular w-4 text-center text-sm font-semibold">{line.qty}</span>
                        <button onClick={() => setQty(line.product.id, line.qty + 1)} className="flex h-7 w-7 items-center justify-center rounded-full bg-secondary text-secondary-foreground" aria-label="mehr"><Plus className="h-3.5 w-3.5" /></button>
                      </div>
                      <span className="tabular w-14 text-right text-sm font-semibold">{formatEuro(line.lineTotal)}</span>
                    </li>
                  ))}
                </ul>
              </article>
              ...
              <article className="surface-card overflow-hidden border border-dashed border-border">
                ...
                <ul>
                  {plan.missing.map((line) => (
                    <li key={line.product.id} className="flex items-center gap-3 border-t border-border px-4 py-2.5">
                      <span className="min-w-0 flex-1 truncate text-sm">{line.product.name}</span>
                      <span className="text-xs text-muted-foreground">Menge {line.qty}</span>
                    </li>
                  ))}
                </ul>
              </article>
          ...
          <p className="px-5 pt-4 text-center text-[11px] text-muted-foreground">
            Preise, Gültigkeit und Fahrtkosten stammen aus deiner aktuellen LocalPrices-Datenbasis.
          </p>
        </>
      )}
    </div>
