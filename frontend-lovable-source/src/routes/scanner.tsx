  ...
  head: () => ({
    meta: [
      { title: "Barcode scannen & Preise vergleichen – LocalPrices" },
      {
        name: "description",
        content:
          "Scanne den Barcode eines Produkts und sieh sofort, welcher Markt in deinem Umkreis den besten Preis hat.",
      },
      { property: "og:title", content: "Barcode-Scanner – LocalPrices" },
      {
        property: "og:description",
        content: "Produkt scannen und Preise aller Märkte im Umkreis vergleichen.",
      },
    ],
  }),
...
function ScannerPage() {
  ...
  const comparison = found
    ? activeIds
        .map((id) => {
          const price = getPrice(found.id, id)!;
          return { market: getMarket(id)!, price, value: effectivePrice(price) };
        })
    ...
    <div>
      ...
      <div className="mx-5 overflow-hidden rounded-3xl bg-foreground/90 shadow-float">
        <div className="relative aspect-[4/3]">
          {scanning ? (
            ...
            <div className="flex h-full flex-col items-center justify-center gap-3 text-background/80">
              ...
              <p className="text-sm">Kamera starten, um zu scannen</p>
            </div>
          )}
        ...
        <button
          ...
          className="w-full bg-primary py-3.5 text-sm font-semibold text-primary-foreground"
        >
          {scanning ? "Scan beenden" : "Kamera starten"}
        </button>
      </div>
      ...
      <div className="no-scrollbar mt-3 flex gap-2 overflow-x-auto px-5">
        {PRODUCTS.slice(0, 6).map((p) => (
          <button
            key={p.id}
            onClick={() => resolve(p.ean)}
            className="shrink-0 rounded-full bg-surface px-3 py-1.5 text-xs text-muted-foreground shadow-card"
          >
      ...
      {found && (
        <section className="mx-5 mt-5 surface-card overflow-hidden">
          <header className="flex items-center gap-3 border-b border-border p-4">
            ...
            <div className="min-w-0 flex-1">
              ...
              <p className="text-xs text-muted-foreground">
                {found.brand} · {found.unit}
              </p>
            </div>
            <button
              onClick={() => {
                addToBasket(found.id);
                toast.success("Zur Liste hinzugefügt");
              }}
          ...
          <ul>
            {comparison.map((c, i) => (
              ...
              >
                ...
                {c.price.offer && (
                  <span className="rounded-full bg-deal/12 px-2 py-0.5 text-[10px] font-bold text-deal">
                    Angebot
                  </span>
                )}
                <span
