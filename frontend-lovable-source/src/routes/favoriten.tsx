...
import {
  fetchFavoriteAlternatives,
  fetchFavoritePreferences,
  persistFavoriteAlternativePreference,
} from "@/services/lokero-state-api";
import {
  fetchFavoriteFamilies,
  fetchProductFamilies,
  setFavoriteFamily,
} from "@/services/lokero-personalization-api";
...
function FavoriteAlternativesBlock({
  productId,
  enabled,
}: {
  productId: string;
  ...
  return (
    <div>
      <p className="mx-3 mt-2 text-[10px] font-semibold uppercase tracking-wide text-muted-foreground">
      ...
      <div className="mt-1 space-y-1">
        {alternatives.data.map((alternative) => (
          <AlternativeCard
            ...
            kind={alternative.kind ?? "aehnlich"}
...
function FavoritesScreen() {
  ...
  const preferences = useQuery({
    queryKey: ["favorite-preferences"],
    queryFn: fetchFavoritePreferences,
  });
  ...
  const favoriteFamilies = useQuery({
    queryKey: ["favorite-families"],
    queryFn: fetchFavoriteFamilies,
  });
  const priceAlertsEnabled = features.data?.features.price_alerts === true;
  const tabs = [
    { id: "produkte", label: "Produkte" },
    { id: "maerkte", label: "Märkte" },
    ...(priceAlertsEnabled ? [{ id: "alarm", label: "Preisalarm" }] : []),
  ];

  const preferenceMap = useMemo(
    () =>
      Object.fromEntries(
        (preferences.data ?? []).map((row) => [row.productId, row.allowAlternatives]),
      ),
    [preferences.data],
  );
  ...
  const grouped = useMemo(
    () =>
      (categories.data ?? [])
        .map((category) => ({
          category,
          rows: rows.filter((row) => getProduct(row.productId)?.category === category.id),
        }))
  ...
  const changeAlternativePreference = async (productId: string, next: boolean) => {
    ...
    toast.success(
      next ? "Alternativen sind für diesen Favoriten erlaubt" : "Alternativen sind deaktiviert",
    );
  ...
  return (
    <div>
      ...
      <div className="bg-surface px-4">
        <Tabs
          options={tabs}
          value={tab === "alarm" && !priceAlertsEnabled ? "produkte" : tab}
          onChange={setTab}
        />
      </div>
                ...
                <div className="flex flex-wrap gap-2">
                  {(favoriteFamilies.data ?? []).map((family) => (
                    <button
                      key={family.slug}
                      type="button"
                      onClick={() => void toggleFamily(family.slug, false)}
                      className="inline-flex items-center gap-2 rounded-full border border-border bg-surface px-3 py-2 text-[12px] font-medium text-navy"
                    >
                      {family.label}
                      <Heart className="h-3.5 w-3.5 fill-discount text-discount" />
                    </button>
                  ))}
                </div>
              ) : (
                <p className="text-[11px] text-muted-foreground">
                  Merke dir z. B. Bier, Cola oder Fisch und erhalte passende Angebote unabhängig von
                  der Marke.
                </p>
              )}
            </div>
            ...
            <div>
              ...
              {products.isSuccess && rows.length === 0 && (
                <EmptyState
                  ...
                  description="Merke dir Produkte, die du regelmäßig kaufst."
                  action={
                    <Link
                      to="/angebote"
                ...
                {grouped.map(({ category, rows: categoryRows }) => (
                  <div key={category.id}>
                    <div className="mb-2 flex items-center justify-between">
                      <h3 className="text-[12px] font-semibold text-navy">{category.label}</h3>
                    ...
                    <div className="space-y-3">
                      {categoryRows.map((f) => {
                        ...
                        const allowAlternatives =
                          preferenceOverrides[f.productId] ?? preferenceMap[f.productId] ?? false;
                        return (
                          <div key={f.productId}>
                            <FavoriteCard
                              product={product}
                              ...
                              onToggleAlternatives={(next) =>
                                void changeAlternativePreference(f.productId, next)
                              }
                            />
                            <FavoriteAlternativesBlock
                              productId={f.productId}
                              enabled={allowAlternatives}
                            />
                          </div>
                        );
                      })}
      ...
      {familyPickerOpen && (
        <div
          className="fixed inset-0 z-50 flex items-end bg-navy/30"
          onClick={() => setFamilyPickerOpen(false)}
        >
          <div
            role="dialog"
            aria-modal="true"
            aria-labelledby="family-picker-title"
            className="w-full rounded-t-3xl bg-surface p-4 pb-[max(1rem,env(safe-area-inset-bottom))]"
            onClick={(event) => event.stopPropagation()}
          >
            <div className="mb-3 flex items-center justify-between">
              <div>
                <h2 id="family-picker-title" className="text-[16px] font-semibold text-navy">
                  Produktfamilie hinzufügen
                </h2>
                <p className="text-[11px] text-muted-foreground">
                  Du bekommst alle passenden lokalen Angebote dieser Produktart.
                </p>
              </div>
              <button
                type="button"
                aria-label="Produktfamilie schließen"
                onClick={() => setFamilyPickerOpen(false)}
              ...
              {(families.data ?? []).map((family) => {
                ...
                return (
                  <button
                    key={family.slug}
                    type="button"
                    onClick={() => void toggleFamily(family.slug, !active)}
                    className={`flex items-center justify-between rounded-xl border px-3 py-3 text-left text-[12px] font-medium ${active ? "border-discount bg-surface text-navy" : "border-border bg-muted-surface text-navy"}`}
                  >
                    <span>{family.label}</span>
                    <Heart className={`h-4 w-4 text-discount ${active ? "fill-discount" : ""}`} />
                  </button>
                );
              })}
            </div>
