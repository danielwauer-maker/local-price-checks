...
def market_identity_admin(
    ...
    standalone = [
        row
        for row in rows
        if row.id not in hidden_ids and mapping[row.id].id == row.id
    ]
    previews = {row.id: preview_false_store_delete(db, row) for row in rows}
...
def delete_false_market(
    ...
    # Resolve the exact requested row, never its canonical alias target.
    store = db.get(Store, store_id)
    if store is None:
        raise HTTPException(404, "Markt nicht gefunden")

    label = f"{store.retailer} | {store.name} | {store.address}"
